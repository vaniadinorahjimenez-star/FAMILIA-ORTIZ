...
export const FamilySyncModal: React.FC<FamilySyncModalProps> = ({
  ...
  const handleTestConnection = async () => {
    ...
    setSyncFeedback('Comprobando enlace con el servidor...');
    const ok = await onCheckConnection();
    setIsCheckingConnection(false);
    if (ok) {
      setSyncFeedback('¡Conexión disponible! Iniciando sincronización...');
      try {
        await onManualSync();
        setSyncFeedback('¡Sincronización completada!');
        soundFX.playFanfare();
      } catch {
        setSyncFeedback('Se detectó red, pero hubo un detalle al sincronizar.');
      }
    } else {
      setSyncFeedback('Aún sin conexión. Los datos siguen guardados localmente.');
      soundFX.playTick();
    }
            ...
            <div className="bg-amber-50 border border-amber-300 rounded-2xl p-3 space-y-2">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <div className="text-xs font-bold text-amber-950 flex items-center gap-1.5 leading-tight">
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all bg-amber-600 hover:bg-amber-700 text-white shadow-2xs active:scale-95 disabled:opacity-70 cursor-pointer"
                >
                  <RefreshCw className={`w-3 h-3 ${isCheckingConnection ? 'animate-spin' : ''}`} />
                  {isCheckingConnection ? 'Comprobando...' : 'Reintentar red'}
                </button>
              </div>

              <div className="bg-white/70 border border-amber-200/80 rounded-xl p-2 text-[11px] text-amber-900 flex items-center justify-between">
                <span className="flex items-center gap-1.5 font-semibold">
                  <HardDrive className="w-3.5 h-3.5 text-amber-700" />
                  {pendingChangesCount > 0
                    ? `${pendingChangesCount} cambios guardados localmente listos para sincronizar`
                    : 'Todas las tareas y rutinas están respaldadas en el navegador'}
                </span>
